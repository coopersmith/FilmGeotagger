...
export function Badges({ frame, compact = false }: { frame: Frame; compact?: boolean }) {
  ...
  if (frame.source === "skipped") items.push({ key: "skip", label: "skipped", glyph: "∅", title: "left unassigned" });
  ...
  else if (!frame.locked) items.push({ key: "interp", label: "interpolated", glyph: "~", title: "placed between its neighbours" });
